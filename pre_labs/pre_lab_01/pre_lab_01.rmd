...
**Answer: id = chr (letters) & seats = dbl (numbers ONLY)**
...
In the space below, write the name of the course from the first row of the "name" column.

**Answer:** **LING889: Directed Research**
...
**Answer: Look at the UMD dataset. Then, take take a look at the "term" column. From there, you can summarize the number of rows, or classes, per term. Label that column "count_classes."**
...
**Answer: The new arrange code rearranged the rows. Now, they are arranged in descending order, from highest class count per term to least.**
...
**Answer: Adding the group_by "department" code added a new column. Class count is now grouped by department per term.**
...
**Answer: We added another summary column. Now, number of classes and available seats are shown per term in descending order.**
...
**Answer: It demonstrated the median and average number of seats per class. The top terms have a similar average around 47 seats per class. The median is lower, indicating there are likely a few outliers significantly impacting the average.**
...
**Answer: Data is now grouped by department and arranged from greatest total number of seats to least. This seems to have been accomplished removing the "term" and using the new "department" group_by code.**
...
1.  Which department teaches the greatest number of courses each semester, on average, including individual sections?

2.  What is an instructor's average courseload in a semester?

3.  Which instructors have taught the most students over time? Courses?
