...
#new_name <- umd_courses |> filter (term > 202500) for filtering with numbers, use greater than
#new_name <- umd_courses |> filter (department == "journalism") for filtering with words and/or specific number, use double equals
...
**Answer: It helps us keep track of what we're doing and keeps us from conflating datasets.**
...
# | means OR inside a filter
# lis win. need to remember boolean operators relevant to data journalists: toolkit mention?
# another language, similar to learning a new database. same ideas, different specifics. 

nrow(and_theatre_seats_15)
```

**Answer: OR enlarged the pool, giving us 54000 results.**
...
# , for partial matches while using R
```
...
**Answer: We mutated the column to lead with the year. The column now follows a year-month-day format.**
...
**Answer: It seems the months are lumped together by year. I believe they are all showing up as the first of the month because that is the format we chose for our dates—not necesarilly because the day is relevant to this answer or what we asked the code to do.**
...
# how do I get it to load the whole thing? please send help. I can't get the spec to work
```
...
**Answer: We created another column based off that total votes column, calculating what percentage Moore received of the total votes.**
...
**Answer: The column should be displayed as percentages since decimals were multiplied by 100. Answers should also be arranged from greatest to least, I think, but I'm not sure because it didn't load.**
...
**Answer: Oh interesting, it looks like it's now arranged from least to most. This is useful for finding the story.**
...
**Answer: Nice! I can str_to_lower, which makes characters lowercase. I can also str_to_title and convert to sentence case. I also like that str_detect can monitor the absence of a pattern.**
...
**Answer: It looks like mutate is finding all of cases where a state name is listed as an address and abbreviating it for consistency.**
...
#I didn't even know that we were covering str_detect when I answered my last question. Again, nice! 
```
...
summary(maryland_expenses)
```

Does this answer make sense to you? Why or why not?

**Answer: This does make sense to me. We calculated the sum of our expenses to other states, which was only made possible by grouping states.**
...
1.  How many UMD courses have 0 seats and no instructor? What percent of UMD's courseload is that?

2.  What is the most common purpose for payments made within Maryland? What committee tends to make these payments?

3.  What was each instructor in the CMNS department's average courseload last semester (or in the last semester we have data for)?
